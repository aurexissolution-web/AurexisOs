// src/components/sections/connect/ConnectQuoteForm.tsx
'use client';
...
const inputClass =
  'w-full rounded-xl border border-white/10 bg-white/[0.04] px-3.5 py-2.5 text-[14px] text-white placeholder:text-white/30 outline-none transition-colors focus-visible:border-[#B08FF0]/60 focus-visible:ring-2 focus-visible:ring-[#B08FF0]/40';

const labelClass = 'font-mono text-[9.5px] uppercase tracking-[0.2em] text-white/45';

const MEETING_META: Record<ConnectMeetingPreference, { icon: ReactNode; hint: string }> = {
...
const STEPS = [
  ...
  'We meet online or face to face and map out your WhatsApp flows.',
];
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      <label className={labelClass} htmlFor={htmlFor}>
        {label} {required && <span style={{ color: CONNECT_ACCENT }}>*</span>}
      </label>
      {children}
...
export function ConnectQuoteForm() {
  const [tier, setTier] = useState<ConnectTier>(QUOTE_TIER_OPTIONS[0]);
  const [businessDescription, setBusinessDescription] = useState('');
  const [hasMetaAccount, setHasMetaAccount] = useState<ConnectHasMetaAccount>(
    QUOTE_HAS_META_OPTIONS[0],
  );
  const [enquiriesPerMonth, setEnquiriesPerMonth] = useState<ConnectEnquiryVolume>(
    QUOTE_ENQUIRY_VOLUME_OPTIONS[0],
  );
  const [timeline, setTimeline] = useState<ConnectTimeline>(QUOTE_TIMELINE_OPTIONS[0]);
  const [budget, setBudget] = useState<ConnectBudget>(QUOTE_BUDGET_OPTIONS[0]);
  const [meeting, setMeeting] = useState<ConnectMeetingPreference>(QUOTE_MEETING_OPTIONS[0]);
  ...
  const [errors, setErrors] = useState<ConnectQuoteFieldErrors>({});
  const [topError, setTopError] = useState<string | null>(null);

  const faceToFace = meeting === 'Face to face';

  function validate(): ConnectQuoteFieldErrors {
    const next: ConnectQuoteFieldErrors = {};
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (res.status === 422) {
        const body = (await res.json()) as { errors?: ConnectQuoteFieldErrors };
  ...
  return (
    <section id="get-a-quote" className="relative overflow-hidden px-6 py-12 md:py-14">
      <div
        ...
        style={{
          background: `radial-gradient(40% 55% at 10% 30%, ${accent(0.12)}, transparent 70%), radial-gradient(40% 50% at 95% 90%, rgba(${CONNECT_DEEP_RGB},0.2), transparent 70%)`,
        }}
      />
      <div
        aria-hidden
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-12">
        <div className="lg:pt-2">
          <ConnectSectionLabel>Get a Quote</ConnectSectionLabel>
          <h2
          ...
          >
            <MessageCircle aria-hidden className="h-4 w-4" style={{ color: CONNECT_ACCENT }} />
        ...
        >
          {state === 'sent' ? (
            ...
            >
              ...
              <Field label="Which Connect tier?" htmlFor="tier">
              ...
              >
                <input
                  ...
                  placeholder="e.g. dental clinic in PJ"
                  className={inputClass}
                />
              </Field>

              <div className="flex flex-col gap-1.5">
                <span className={labelClass} id="hasMeta-label">
                  Meta Business account today?
                </span>
                <div
                  role="radiogroup"
                  aria-labelledby="hasMeta-label"
                  className="flex flex-wrap gap-2"
                >
                  {QUOTE_HAS_META_OPTIONS.map((opt) => {
                    const active = opt === hasMetaAccount;
                    return (
                      <button
                        key={opt}
                        type="button"
                        role="radio"
                        aria-checked={active}
                        onClick={() => setHasMetaAccount(opt)}
                        className="rounded-full px-4 py-2 text-[13px] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#B08FF0]/50"
                        style={{
                          border: `1px solid ${active ? CONNECT_ACCENT : 'rgba(255,255,255,0.12)'}`,
                          background: active ? CONNECT_ACCENT : 'rgba(255,255,255,0.03)',
              ...
              <Field label="Enquiries per month" htmlFor="enquiriesPerMonth">
                <Select
                  id="enquiriesPerMonth"
                  value={enquiriesPerMonth}
                  options={QUOTE_ENQUIRY_VOLUME_OPTIONS}
                  onChange={setEnquiriesPerMonth}
                />
              </Field>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {QUOTE_MEETING_OPTIONS.map((opt) => {
                    ...
                    return (
                      <label
                        key={opt}
                        className="flex cursor-pointer items-center gap-3 rounded-xl border px-3.5 py-2.5 transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-[#B08FF0]/50"
                        style={{
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{
                            ...
                            color: active ? CONNECT_ACCENT : 'rgba(255,255,255,0.55)',
                          }}
                        >
              ...
              <Field label="Anything else? (optional)" htmlFor="notes">
                <input
                  ...
                  placeholder="Current setup, volumes…"
                  className={inputClass}
                />
              </Field>
              ...
              <button
                ...
                style={{ background: CONNECT_ACCENT, boxShadow: `0 0 30px ${accent(0.3)}` }}
              >
