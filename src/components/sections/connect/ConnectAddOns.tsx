// src/components/sections/connect/ConnectAddOns.tsx
...
const PIECE_ICONS = [Rocket, Languages, CalendarCheck, Handshake];

const ITEM_ICONS: Record<string, LucideIcon> = {
  'Extra language': Languages,
  'Extra number or branch': Split,
  'CRM integration': Contact,
  'Booking integration': CalendarCheck,
  'Payment collection': CreditCard,
  'Click-to-WhatsApp ads setup': Megaphone,
  'Product catalogue': ShoppingBag,
  'Instagram and Messenger': Instagram,
  'Data import': Upload,
  'Team training': GraduationCap,
};

export function ConnectAddOns() {
  return (
    <section className="relative overflow-hidden px-6 py-12 md:py-14">
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <Image
          src="/images/connect/addons-bg.webp"
          alt=""
          fill
          sizes="100vw"
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="grid items-end gap-6 lg:grid-cols-[1.2fr_1fr]">
          <div>
            <ConnectSectionLabel>Add-ons</ConnectSectionLabel>
            <h2
          ...
          <p className="text-[15px] leading-[1.6] text-white/55 lg:pb-2">
            Add to any Connect package. One-off unless stated.
          </p>
        </div>
        ...
        <div
          ...
          style={{
            ...
            background: `radial-gradient(70% 120% at 0% 0%, ${accent(0.16)}, transparent 65%), #0C0818`,
          }}
        >
          <div className="relative grid items-center gap-6 lg:grid-cols-[1fr_auto] lg:gap-10">
            <div>
              <span
                ...
                style={{ background: accent(0.16), color: CONNECT_ACCENT }}
              >
                Most popular bundle
              </span>
            ...
            <ol className="grid grid-cols-2 gap-2 sm:flex sm:items-stretch md:gap-3">
              {BUNDLE_PIECES.map((piece, i) => {
                const Icon = PIECE_ICONS[i] ?? Rocket;
                return (
                  <li key={piece} className="flex items-center gap-2 md:gap-3">
                    ...
                    <span
                      className="flex h-full w-full flex-col items-center gap-2 rounded-2xl border bg-[#0D0A1A] px-2 py-3 text-center sm:w-[108px]"
                      style={{
                    ...
                    >
                      <span
                        ...
                        style={{ background: accent(i === 0 ? 0.22 : 0.1), color: CONNECT_ACCENT }}
                      >
        ...
        >
          {ADD_ONS.map((item, i) => {
            const Icon = ITEM_ICONS[item.name] ?? Plus;
            return (
              <li
                key={item.name}
