// src/components/sections/core/CoreCarePlans.tsx
  ...
  return (
    <section className="relative overflow-hidden px-6 py-12 md:py-14">
      <div
        ...
        style={{
          background: `radial-gradient(40% 45% at 85% 20%, ${accent(0.1)}, transparent 70%), radial-gradient(40% 50% at 5% 90%, rgba(${CORE_DEEP_RGB},0.18), transparent 70%)`,
        }}
      />
      <div
        aria-hidden
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="grid items-end gap-5 lg:grid-cols-[1.2fr_1fr]">
          <div>
            <CoreSectionLabel>Care Plans</CoreSectionLabel>
            <h2
          ...
          <p className="text-[15px] leading-[1.6] text-white/55 lg:pb-1.5">{CARE_PLANS_INTRO}</p>
        </div>

        <div className="mt-8 grid grid-cols-1 gap-3 md:grid-cols-3">
          {CARE_TIERS.map((tier, idx) => {
            const level = idx + 1;
                  ...
                  <span className="flex gap-1" aria-label={`Level ${level} of ${levels}`}>
                    {Array.from({ length: levels }, (_, i) => (
                      <span
                        ...
                        style={{
                          background: i < level ? CORE_ACCENT : 'rgba(255,255,255,0.12)',
                        }}
                      />
                    ))}
                  </span>
                ...
                <div className="mt-4 border-t border-white/[0.08] pt-4">
                  {inherits && (
                    <p
                      ...
                      style={{ background: accent(0.12), color: CORE_ACCENT }}
                    >
        ...
        <div className="mt-4 space-y-1.5">
          <p className="max-w-3xl text-[12.5px] leading-[1.6] text-white/40">{CARE_FOOTNOTE}</p>
          <p className="max-w-3xl text-[12.5px] leading-[1.6] text-white/40">
            {CARE_AI_UPLIFT_NOTE}
          </p>
        </div>
      </div>
