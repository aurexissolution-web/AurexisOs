// src/components/sections/core/CoreQuoteForm.tsx
'use client';
...
const inputClass =
  'w-full rounded-xl border border-white/10 bg-white/[0.04] px-3.5 py-2.5 text-[14px] text-white placeholder:text-white/30 outline-none transition-colors focus-visible:border-[#8FA8F0]/60 focus-visible:ring-2 focus-visible:ring-[#8FA8F0]/40';

const labelClass = 'font-mono text-[9.5px] uppercase tracking-[0.2em] text-white/45';

const MEETING_META: Record<CoreMeetingPreference, { icon: ReactNode; hint: string }> = {
...
const STEPS = [
  ...
  'We run a discovery session — online or face to face — then send a written scope.',
];
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      <label className={labelClass} htmlFor={htmlFor}>
        {label} {required && <span style={{ color: CORE_ACCENT }}>*</span>}
      </label>
      {children}
...
export function CoreQuoteForm() {
  const [tier, setTier] = useState<CoreTier>(QUOTE_TIER_OPTIONS[0]);
  const [businessDescription, setBusinessDescription] = useState('');
  const [saasSpend, setSaasSpend] = useState<CoreSaasSpend>(QUOTE_SAAS_SPEND_OPTIONS[0]);
  const [bottleneck, setBottleneck] = useState<CoreBottleneck>(QUOTE_BOTTLENECK_OPTIONS[0]);
  const [dataMigration, setDataMigration] = useState<CoreDataMigration>(
    QUOTE_DATA_MIGRATION_OPTIONS[0],
  );
  const [timeline, setTimeline] = useState<CoreTimeline>(QUOTE_TIMELINE_OPTIONS[0]);
  const [budget, setBudget] = useState<CoreBudget>(QUOTE_BUDGET_OPTIONS[0]);
  const [meeting, setMeeting] = useState<CoreMeetingPreference>(QUOTE_MEETING_OPTIONS[0]);
  ...
  const [errors, setErrors] = useState<CoreQuoteFieldErrors>({});
  const [topError, setTopError] = useState<string | null>(null);

  const faceToFace = meeting === 'Face to face';

  function validate(): CoreQuoteFieldErrors {
    const next: CoreQuoteFieldErrors = {};
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (res.status === 422) {
        const body = (await res.json()) as { errors?: CoreQuoteFieldErrors };
  ...
  return (
    <section id="get-a-quote" className="relative overflow-hidden px-6 py-12 md:py-14">
      <div
        ...
        style={{
          background: `radial-gradient(40% 55% at 10% 30%, ${accent(0.12)}, transparent 70%), radial-gradient(40% 50% at 95% 90%, rgba(${CORE_DEEP_RGB},0.2), transparent 70%)`,
        }}
      />
      <div
        aria-hidden
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-12">
        <div className="lg:pt-2">
          <CoreSectionLabel>Get a Quote</CoreSectionLabel>
          <h2
          ...
          <p className="mt-4 max-w-sm text-[15px] leading-[1.6] text-white/55">
            Tell us what you need. We&apos;ll WhatsApp you within a business day to book a discovery
            call.
          </p>
          <p className="mt-3 max-w-sm text-[12.5px] leading-[1.55] text-white/40">
            {QUOTE_CAPACITY_NOTE}
          </p>
          ...
          >
            <MessageCircle aria-hidden className="h-4 w-4" style={{ color: CORE_ACCENT }} />
        ...
        >
          {state === 'sent' ? (
            ...
            >
              ...
              <Field label="Which Core tier?" htmlFor="tier">
              ...
              >
                <input
                  ...
                  placeholder="e.g. multi-branch dental group in Klang Valley"
                  className={inputClass}
                />
              </Field>

              <Field label="Monthly software spend" htmlFor="saasSpend">
                <Select
                  id="saasSpend"
                  value={saasSpend}
                  options={QUOTE_SAAS_SPEND_OPTIONS}
                  onChange={setSaasSpend}
                />
              </Field>

              <Field label="Biggest bottleneck today" htmlFor="bottleneck">
                <Select
                  id="bottleneck"
                  value={bottleneck}
                  options={QUOTE_BOTTLENECK_OPTIONS}
                  onChange={setBottleneck}
                />
              </Field>

              <div className="flex flex-col gap-1.5 sm:col-span-2">
                <span className={labelClass} id="dataMigration-label">
                  Existing data to migrate?
                </span>
                <div
                  role="radiogroup"
                  aria-labelledby="dataMigration-label"
                  className="flex flex-wrap gap-2"
                >
                  {QUOTE_DATA_MIGRATION_OPTIONS.map((opt) => {
                    const active = opt === dataMigration;
                    return (
                      <button
                        key={opt}
                        type="button"
                        ...
                        className="rounded-full px-3 py-1.5 text-[12px] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8FA8F0]/50"
                        style={{
                          border: `1px solid ${active ? CORE_ACCENT : 'rgba(255,255,255,0.12)'}`,
                          background: active ? CORE_ACCENT : 'rgba(255,255,255,0.03)',
              ...
              <Field label="Rough budget for setup" htmlFor="budget">
                <Select
                  id="budget"
                  value={budget}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {QUOTE_MEETING_OPTIONS.map((opt) => {
                    ...
                    return (
                      <label
                        key={opt}
                        className="flex cursor-pointer items-center gap-3 rounded-xl border px-3.5 py-2.5 transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-[#8FA8F0]/50"
                        style={{
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{
                            ...
                            color: active ? CORE_ACCENT : 'rgba(255,255,255,0.55)',
                          }}
                        >
              ...
              <Field label="Anything else? (optional)" htmlFor="notes">
                <input
                  ...
                  placeholder="Current systems, team size…"
                  className={inputClass}
                />
              </Field>
              ...
              <button
                ...
                style={{ background: CORE_ACCENT, boxShadow: `0 0 30px ${accent(0.3)}` }}
              >
