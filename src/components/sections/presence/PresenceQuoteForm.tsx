// src/components/sections/presence/PresenceQuoteForm.tsx
'use client';
...
const inputClass =
  'w-full rounded-xl border border-white/10 bg-white/[0.04] px-3.5 py-2.5 text-[14px] text-white placeholder:text-white/30 outline-none transition-colors focus-visible:border-[#5EE3DA]/60 focus-visible:ring-2 focus-visible:ring-[#5EE3DA]/40';

const labelClass = 'font-mono text-[9.5px] uppercase tracking-[0.2em] text-white/45';

const MEETING_META: Record<QuoteMeetingPreference, { icon: ReactNode; hint: string }> = {
...
const STEPS = [
  ...
  'We meet online or face to face and send a fixed quote.',
];
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      <label className={labelClass} htmlFor={htmlFor}>
        {label} {required && <span style={{ color: PRESENCE_ACCENT }}>*</span>}
      </label>
      {children}
...
export function PresenceQuoteForm() {
  const [websiteType, setWebsiteType] = useState<QuoteWebsiteType>(QUOTE_WEBSITE_TYPE_OPTIONS[0]);
  const [businessDescription, setBusinessDescription] = useState('');
  const [hasWebsite, setHasWebsite] = useState<QuoteHasWebsite>(QUOTE_HAS_WEBSITE_OPTIONS[0]);
  const [timeline, setTimeline] = useState<QuoteTimeline>(QUOTE_TIMELINE_OPTIONS[0]);
  const [budget, setBudget] = useState<QuoteBudget>(QUOTE_BUDGET_OPTIONS[0]);
  const [meeting, setMeeting] = useState<QuoteMeetingPreference>(QUOTE_MEETING_OPTIONS[0]);
  ...
  const [errors, setErrors] = useState<PresenceQuoteFieldErrors>({});
  const [topError, setTopError] = useState<string | null>(null);

  const faceToFace = meeting === 'Face to face';

  function validate(): PresenceQuoteFieldErrors {
    const next: PresenceQuoteFieldErrors = {};
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (res.status === 422) {
        const body = (await res.json()) as { errors?: PresenceQuoteFieldErrors };
  ...
  return (
    <section id="get-a-quote" className="relative overflow-hidden px-6 py-12 md:py-14">
      <div
        ...
        style={{
          background: `radial-gradient(40% 55% at 10% 30%, ${accent(0.12)}, transparent 70%), radial-gradient(40% 50% at 95% 90%, rgba(0,110,140,0.12), transparent 70%)`,
        }}
      />
      <div
        aria-hidden
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-12">
        <div className="lg:pt-2">
          <PresenceSectionLabel>Get a Quote</PresenceSectionLabel>
          <h2
          ...
          >
            <MessageCircle aria-hidden className="h-4 w-4" style={{ color: PRESENCE_ACCENT }} />
        ...
        >
          {state === 'sent' ? (
            ...
            >
              ...
              <Field label="Which type of website?" htmlFor="websiteType">
                <Select
                  id="websiteType"
                  value={websiteType}
                  options={QUOTE_WEBSITE_TYPE_OPTIONS}
              ...
              <div className="flex flex-col gap-1.5 sm:col-span-2">
                <span className={labelClass} id="hasWebsite-label">
                  Do you have a website today?
                </span>
                <div
                  role="radiogroup"
                  aria-labelledby="hasWebsite-label"
                  className="flex flex-wrap gap-2"
                >
                  {QUOTE_HAS_WEBSITE_OPTIONS.map((opt) => {
                    const active = opt === hasWebsite;
                    return (
                      <button
                        key={opt}
                        type="button"
                        ...
                        className="rounded-full px-4 py-2 text-[13px] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5EE3DA]/50"
                        style={{
                          border: `1px solid ${active ? PRESENCE_ACCENT : 'rgba(255,255,255,0.12)'}`,
                          background: active ? PRESENCE_ACCENT : 'rgba(255,255,255,0.03)',
                ...
                <div className="grid grid-cols-2 gap-2">
                  {QUOTE_MEETING_OPTIONS.map((opt) => {
                    ...
                    return (
                      <label
                        key={opt}
                        className="flex cursor-pointer items-center gap-3 rounded-xl border px-3.5 py-2.5 transition-colors has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-[#5EE3DA]/50"
                        style={{
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{
                            ...
                            color: active ? PRESENCE_ACCENT : 'rgba(255,255,255,0.55)',
                          }}
                        >
              ...
              <Field label="Anything else? (optional)" htmlFor="notes">
                <input
                  ...
                  placeholder="Pages you need, sites you like…"
                  className={inputClass}
                />
              </Field>
              ...
              <button
                ...
                style={{ background: PRESENCE_ACCENT, boxShadow: `0 0 30px ${accent(0.3)}` }}
              >
