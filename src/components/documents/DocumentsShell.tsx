...
// src/components/documents/DocumentsShell.tsx
// Same frame as the admin panel (sidebar, backdrop, mobile tab bar), for the
// private Documents dashboard.
import Link from 'next/link';
import Image from 'next/image';
import { usePathname } from 'next/navigation';
import type { ReactNode } from 'react';
import { ExternalLink, FileText, LayoutDashboard, LogOut, Receipt } from 'lucide-react';
import { cn } from '@/lib/utils';
import { docsSignOut } from '@/app/documents/access-actions';
import { ToastProvider } from '@/components/admin/ui';
import { DarkGradientBg } from '@/components/admin/DarkGradientBg';

const NAV = [
  { href: '/documents', label: 'Overview', short: 'Overview', icon: LayoutDashboard },
  { href: '/documents/proposals', label: 'Proposals', short: 'Proposals', icon: FileText },
  { href: '/documents/billing', label: 'Invoices & Receipts', short: 'Billing', icon: Receipt },
] as const;

export function DocumentsShell({ user, children }: { user: { name: string; email: string }; children: ReactNode }) {
  const pathname = usePathname();
  const isActive = (href: string) => (href === '/documents' ? pathname === '/documents' : pathname.startsWith(href));
    ...
    <ToastProvider>
      <div className="relative min-h-screen bg-black text-white">
        ...
        <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col border-r border-white/[0.12] bg-[#04060A]/80 backdrop-blur-2xl lg:flex">
          <Link href="/documents" className="flex items-center gap-3 px-5 pb-6 pt-6">
            ...
            <span>
              ...
              <span className="block font-mono text-[9px] uppercase tracking-[0.28em] text-[#5EE3DA]">Documents</span>
            </span>
          </Link>

          <nav className="flex-1 space-y-5 px-3" aria-label="Documents">
            <div>
              <p className="mb-1.5 px-3 font-mono text-[9.5px] uppercase tracking-[0.24em] text-white/30">Create</p>
        ...
        <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-white/[0.12] bg-[#05070C]/85 px-4 backdrop-blur-xl lg:hidden">
          <Link href="/documents" className="flex items-center gap-2.5">
            ...
            <span className="text-[14px] font-bold">Aurexis Documents</span>
          </Link>
          <form action={docsSignOut}>
        ...
        <nav
          ...
          className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-3 border-t border-white/[0.14] bg-[#05070C]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl lg:hidden"
        >
          {NAV.map((item) => {
            const active = isActive(item.href);
            return (
